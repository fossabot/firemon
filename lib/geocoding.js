...
exports.nearestCities = function(lat, lon, count, maxKm) {
  ...
  return i.map(x => {x['distance'] = geo.distance(lon, lat, x.lon, x.lat); return x;})
};
...
exports.cityDisplayName = function(x) {
  const elems = [x.name, x.adminCode ? (exports.statesByAbbrev[x.adminCode] || x.adminCode) : null, x.country].filter(x => x ? true : false)
  return elems.join(', ');
}
