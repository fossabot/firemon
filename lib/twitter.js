...
const twitterDaemon = function(path, semaphore) {
  ...
  const postLoop = function() {
    fs.readdir(path, function(err, items2) {
        ...
        if (items.length > 0) {
          ...
          if (item.image1) {
            semaphore.take(() => {
              twitterAccount.post('media/upload', {media_data: fs.readFileSync(item.image1, {encoding: 'base64'})}, function(err, data, resp) {
                if (err) {
                  ...
                  twitterAccount.post('media/metadata/create', metadata, function (err, data, resp) {
                    if (err) {
                      ...
                      if (item.image2) {
                        twitterAccount.post('media/upload', {media_data: fs.readFileSync(item.image2, {encoding: 'base64'})}, function(err, data, resp) {
                          if (err) {
                            console.log(' !!! Could not upload image from ' + p);
                            console.log(err);
                            semaphore.leave();
